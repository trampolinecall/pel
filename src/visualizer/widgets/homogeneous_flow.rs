...
pub(crate) struct FlowRenderObject<Data, Child: RenderObject<Data>> {
    direction: Direction,
    children: Vec<(graphics::Vector2f, Child)>,
...
impl<Data, Child: Widget<Data>> Flow<Data, Child> {
    pub(crate) fn new(direction: Direction, children: Vec<Child>) -> Self {
        Self { direction, children, _phantom: PhantomData }
    }
    pub(crate) fn new_horizontal(children: Vec<Child>) -> Self {
        Self::new(Direction::Horizontal, children)
    }
    pub(crate) fn new_vertical(children: Vec<Child>) -> Self {
...
impl<Data, Child: Widget<Data>> Widget<Data> for Flow<Data, Child> {
    ...
    fn to_render_object(self, id_maker: &mut RenderObjectIdMaker) -> Self::Result {
        FlowRenderObject {
            ...
            children: self.children.into_iter().map(|child| (graphics::Vector2f::new(0.0, 0.0), child.to_render_object(id_maker))).collect(),
    ...
    fn update_render_object(self, render_object: &mut Self::Result, id_maker: &mut RenderObjectIdMaker) {
        ...
        let new_ro_children = self
            ...
            .map(|(widget, ro)| match ro {
                Some((offset, mut ro)) => {
                    widget.update_render_object(&mut ro, id_maker);
                    (offset, ro)
                }
                None => (graphics::Vector2f::new(0.0, 0.0), widget.to_render_object(id_maker)),
            })
            .collect();
...
impl<Data, Child: RenderObject<Data>> RenderObject<Data> for FlowRenderObject<Data, Child> {
    fn layout(&mut self, graphics_context: &graphics::GraphicsContext, sc: layout::SizeConstraints) {
        ...
        for (offset, child) in &mut self.children {
            *offset = self.direction.make_vector_in_direction(current_offset, 0.0);
            current_offset += self.direction.take_major_component(child.size());
            let child_minor_size = self.direction.take_minor_component(child.size());
            max_minor_size = if child_minor_size > max_minor_size { child_minor_size } else { max_minor_size };
        }
        self.own_size = self.direction.make_vector_in_direction(current_offset, max_minor_size);
    }
